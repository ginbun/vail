...
<script lang="ts" setup>
  import { TerminalTabs, TerminalShortcutKeys } from '../../types/const';
  ...
  const { layoutState, preference, tabManager, getCurrentDomViewportHandler, sessionManager } = useTerminalStore();
  ...
  watch(() => tabManager.active, (active, before) => {
    ...
    if (before === TerminalTabs.TERMINAL_PANEL.key) {
      getCurrentDomViewportHandler()?.blur?.();
    }
    // 聚焦 tab
