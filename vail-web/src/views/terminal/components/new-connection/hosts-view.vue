...
v-else
    ...
    <!-- 主机额外设置模态框 -->
    <host-extra-modal ref="extraModal" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import { NewConnectionType, openExtraModalKey } from '../../types/const';
  import HostGroupView from './host-group-view.vue';
  import HostListView from './host-list-view.vue';
  import HostExtraModal from '../setting/extra/host-extra-modal.vue';
  ...
  // 暴露打开额外配置模态框
  provide(openExtraModalKey, (record: any) => {
    extraModal.value?.open(record);
  });
