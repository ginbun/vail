...
export const titleFilter = (searchValue: string, option: TreeNodeData) => {
  return String(option.title || '').toLowerCase().includes(searchValue.toLowerCase());
};
...
export const tagLabelFilter = (searchValue: string, option: SelectOptionData) => {
  if (searchValue.startsWith('@') && option.isTag) {
    // tag 过滤
    return String(option.label || '').toLowerCase().startsWith(searchValue.substring(1, searchValue.length).toLowerCase());
  } else {
    // 文本过滤
    return String(option.label || '').toLowerCase().includes(searchValue.toLowerCase());
  }
